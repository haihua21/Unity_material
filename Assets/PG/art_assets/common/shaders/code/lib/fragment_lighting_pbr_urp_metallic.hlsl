...
#include "Assets/Packages/LocalPackages/ShaderLibrary/Lighting.hlsl"
#include "lighting_direct.hlsl"
#include "lighting_indirect.hlsl"
#include "simple_data.hlsl"


half3 LightingGI(InputData inputData, SurfaceData surfaceData, SimpleData simpleData, half3 lightColor){
    half3 color = half3(0,0,0);
#if defined(_GI_DIFFUSE)
    color = inputData.bakedGI * simpleData.diffuse * _IndirectDiffuseColor * _IndirectDiffuseIntensity;
#endif

#if defined(_GI_SPECULAR)

    half3 indirect_color = LightingGI_Specular_CubeMap(inputData.normalWS, inputData.viewDirectionWS) * surfaceData.metallic * lightColor;
    indirect_color = LightingGI_Specular_Matcap(simpleData.reflection_specular, inputData.normalWS);

#if !defined(_REFLECTION_Cube) && !defined(_MATCAP)
    indirect_color = LightingGI_BRDF_Specular(simpleData.specular, simpleData.perceptualRoughness, simpleData.roughness2, simpleData.grazingTerm, 
                                        surfaceData.occlusion, inputData.normalWS, inputData.viewDirectionWS);
#endif

    color += indirect_color * _IndirectSpecularColor * _IndirectSpecularIntensity;
...
half4 UniversalFragmentSimple(InputData inputData, SurfaceData surfaceData)
{
    ...
    indirect_color = lerp(indirect_color, indirect_color * radiance, _GI_ShadowIntensity);
    ...
    //return half4(_WeatherColor.rgb, 1);
}
