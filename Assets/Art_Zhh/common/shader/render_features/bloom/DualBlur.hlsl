...
#endif
	//
	o.uv[1] = v.uv + float2(-1, -1)  * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5; //↖
	o.uv[2] = v.uv + float2(-1,  1)  * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5; //↙
	o.uv[3] = v.uv + float2(1,  -1)  * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5; //↗
	o.uv[4] = v.uv + float2(1,   1)  * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5; //↘
	//
    return o;
    //5 samples，组成一个五筒
}
...
v2f_DualBlurUp DualBlurUpVert(appdata v)
{
    ...
    o.vertex = TransformObjectToHClip(v.vertex.xyz);
	//
	o.uv[0] = v.uv + float2(-1,-1) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
	o.uv[1] = v.uv + float2(-1, 1) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
	o.uv[2] = v.uv + float2(1, -1) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
	o.uv[3] = v.uv + float2(1,  1) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
	o.uv[4] = v.uv + float2(-2, 0) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
	o.uv[5] = v.uv + float2(0, -2) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
	o.uv[6] = v.uv + float2(2,  0) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
	o.uv[7] = v.uv + float2(0,  2) * (1 + _BloomRange) * _MainTex_TexelSize.xy * 0.5;
    //
    return o;
}
