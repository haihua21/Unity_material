...
float4 _Params; // x: scatter, y: clamp, z: threshold (linear), w: threshold knee
  ...
  half4 EncodeHDR(half3 color)
        ...
        half3 Upsample(float2 uv)
        {
            ...
            half3 lowMip = DecodeHDR(SampleTexture2DBicubic(TEXTURE2D_X_ARGS(_MainTex, sampler_LinearClamp), uv, _MainTex_TexelSize.zwxy, (1.0).xx, unity_StereoEyeIndex));
        #else
            half3 lowMip = DecodeHDR(SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv));
        #endif
