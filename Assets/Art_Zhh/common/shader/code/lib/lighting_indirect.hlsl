...
half3 LightingGI_Specular_Matcap(half3 specular, float3 normalWS, half perceptualRoughness = 0){
#if defined(_MATCAP)
    const uint matcapMipSteps = 8;//for 256x256, has 9 mips
    half mip = PerceptualRoughnessToMipmapLevel(perceptualRoughness, matcapMipSteps);
    ...
    return SAMPLE_TEXTURE2D_LOD(_Matcap, sampler_Matcap, matcap_uv, mip).xyz * _MatcapIntensity * specular * _MatcapColor.rgb;
    // return SAMPLE_TEXTURE2D_LOD(_Matcap, sampler_Matcap, matcap_uv, 0).xyz * _MatcapIntensity * specular * _MatcapColor.rgb;
#endif
