Shader "Code/HairPBR"
{
    ...
    SubShader
    {
        ...
        inline float Dither4x4(float2 screenUV)
        {
            const float dither[16] = {1,9,3,11, 13,5,15,7, 4,12,2,10, 16,8,14,6};
            int2 coord = int2(fmod(screenUV.x, 4), fmod(screenUV.y, 4));
            return dither[coord.y * 4 + coord.x] / 16.0;
        }
        ...
        Pass
        {
            ...
            half4 frag(HairVertexOutput IN, bool isFrontFace : SV_IsFrontFace) : SV_Target
            {
                ...
                float baseAlpha = baseTex.a * _BaseColor.a;
                ...
                float dither = Dither4x4(screenUV);
                float alpha = baseAlpha + lerp(dither, 1.0, step(_DitherThreshold, baseAlpha)) - 0.5;
        ...
        Pass
        {
            ...
            ShadowVertexOutput ShadowVert(ShadowVertexInput v)
            {
                ...
                float3 worldPos = TransformObjectToWorld(v.vertex.xyz);
                float3 normalWS = TransformObjectToWorldNormal(v.vertex.xyz);
            ...
            half4 ShadowFrag(ShadowVertexOutput IN) : SV_TARGET
            {
                UNITY_SETUP_INSTANCE_ID(IN);
